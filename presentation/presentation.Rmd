...
# Scatterplot avec les deux races différenciées par couleur
ggplot(eggdata_grouped, aes(x = `Quantité de nourriture moyenne (g)`, y = `Production moyenne d'œufs par jour`, color = `Race de poule`)) +
  geom_point(alpha = 0.7, size = 3) +
  labs(
    title = "Relation entre la quantité de nourriture et la production moyenne d'œufs par race",
...
---
```{r echo=FALSE}
# Calcul de la production moyenne et de l'exposition moyenne par poule
eggdata_grouped <- eggdata %>%
  group_by(`GallusID`,`Race de poule`) %>%
  summarise(
    `Exposition moyenne au soleil (h)` = mean(`Exposition au soleil (h)`, na.rm = TRUE),
    `Production moyenne d'œufs par jour` = mean(`Production d'œufs par jour`, na.rm = TRUE)
  )

# Création des graphiques par race
ggplot(eggdata_grouped, aes(x = as.factor(`Exposition moyenne au soleil (h)`), 
                            y = `Production moyenne d'œufs par jour`, 
                            fill = `Race de poule`)) +
  geom_boxplot() +
  facet_wrap(~ `Race de poule`, ncol = 1) +  # Facette pour séparer les races verticalement
  labs(
    title = "Impact de l'exposition moyenne au soleil sur la production moyenne d'œufs",
